#!/usr/bin/env bash
#
# doctor.sh — check that the store agrees with all three skill manifests.
#
# Skills live in .agents/skills/<name>/ (a dir with SKILL.md). Each is tracked
# by exactly one manifest: skills-lock.json (npx), authored.txt (self-written),
# or external.json (GitHub clone + symlink). doctor reconciles the dirs against
# all three and reports drift:
#
#   - orphan dir      : a skill dir tracked by none of the manifests.
#   - missing dir     : a lockfile entry with no dir (npx experimental_install).
#   - missing external: an external.json entry with no resolvable dir (sync).
#   - stale authored  : an authored.txt name with no dir.
#
# Dirs without SKILL.md (dev workspaces) are ignored. Exits non-zero on any
# orphan/missing, so it can gate commits / CI.
#
# Usage:
...
locked="$(lock_skill_names "$lock_file" | sort)"
...
authored="$(read_authored "$authored_file" | sort -u)"
