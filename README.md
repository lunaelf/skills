...
scripts/
  ...
  ui/                    # 本地 Web UI（零依赖：python3 标准库 + 静态页面）
    serve.sh             # 启动入口：生成一次性 token、起服务、开浏览器
    server.py            # 读 manifest 出状态；所有变更调上面的脚本执行
    index.html           # 单页界面（面板 + 操作 + 活动日志）
...
不想记命令时，用本地 Web UI 完成上面的日常操作（`npx skills add` 除外——它是外部
CLI，不在本仓库脚本之列）：

```bash
scripts/ui/serve.sh                # 起服务并打开浏览器（Ctrl-C 停止）
scripts/ui/serve.sh --port 8000    # 指定端口；--no-open 只打印 URL
```

页面上能看到：skill 全量清单（按来源着色，orphan 高亮并给修复按钮）、每个项目
链接了哪些 skill（悬空的标黄）、doctor / `PACKAGES.md` 健康状态。操作上支持
多选链接（含 GLOBAL 和新路径补全）、unlink / prune（先展示脚本 `-n` dry-run
的真实输出再确认）、添加/移除/同步外部 skill、注册项目等；每次操作的脚本原始
输出都在页面底部的活动日志里。

实现上没有第二份逻辑：读取直接来自三个清单文件，所有变更都是调 `scripts/` 里
的脚本执行。服务只绑 `127.0.0.1`，URL 带一次性 token，参数走白名单校验。
